...
pub fn create(device: vk.DeviceProxy, vma: c.VmaAllocator, usage_flags: vk.ImageUsageFlags) !AllocatedImage {
    const video_mode = glfw.getVideoMode(glfw.getPrimaryMonitor()).*;
    const extent = c.VkExtent3D{
        .width = video_mode.width,
        .height = video_mode.height,
        .depth = 1,
    };
    std.log.info(": [{d}x{d}]", .{ extent.width, extent.height });
